...
func CreateDynamicNode(asn uint64, id uint32, meanLatency float64, meanTxUtil float64) DynamicNode {
	return DynamicNode{
		asn:        asn,
		id:         id,
		meanLat:    meanLatency,
		meanTxUtil: meanTxUtil,
	}
}

func (node *DynamicNode) AppendMetadata(ingressIf uint16, egressIf uint16, rep *report.Report) {
	var iface, lat, util [4]byte

	binary.BigEndian.PutUint16(iface[:2], ingressIf)
	binary.BigEndian.PutUint16(iface[2:], egressIf)

	latStdDev := 1000.0
	latency := uint32(math.Min(math.Max(
		latStdDev*rand.NormFloat64()+node.meanLat, 0), math.MaxUint32))
	txUtilStdDev := 0.01 * float64(math.MaxUint32)
	txUtil := uint32(math.Min(math.Max(
		txUtilStdDev*rand.NormFloat64()+node.meanTxUtil, 0), math.MaxUint32))
	binary.BigEndian.PutUint32(lat[:], latency)
	binary.BigEndian.PutUint32(util[:], txUtil)
	...
	hop := report.Hop{
		...
		Metadata: map[uint32][]byte{
			uint32(report.MetadataType_INTERFACE_LEVEL1):      iface[:],
			uint32(report.MetadataType_HOP_LATENCY):           lat[:],
			uint32(report.MetadataType_EGRESS_TX_UTILIZATION): util[:],
		},
	}
	rep.Hops = append(rep.Hops, &hop)
}
...
func (flow *Flow) start(wg *sync.WaitGroup, reportPeriod time.Duration) {
	wg.Add(1)
	go func() {
		defer wg.Done()
		...
		for i := 0; i < flow.pktCount; i++ {
			...
			t1 := time.Now()
			time.Sleep(reportPeriod - t1.Sub(t0))
		}
	}()
}
...
func main() {
	...
	flag.IntVar(&pktsPerFlow, "pkts-per-flow", 10, "Reports generated per flow.")
	flag.IntVar(&reportRate, "rate", 2, "Reports per second and flow.")
	...
	// Prepare topology
	node1 := CreateDynamicNode(0xff00_0000_0001, 1, 10000, math.MaxUint32/4)
	node2 := CreateDynamicNode(0xff00_0000_0002, 1, 20000, math.MaxUint32/2)
	node3 := CreateDynamicNode(0xff00_0000_0003, 1, 50000, math.MaxUint32/8)
	node4 := CreateDynamicNode(0xff00_0000_0003, 2, 80000, math.MaxUint32/8)
	node5 := CreateDynamicNode(0xff00_0000_0004, 1, 15000, math.MaxUint32/2)
	node6 := CreateDynamicNode(0xff00_0000_0005, 1, 50000, math.MaxUint32/2)
	...
	flows := []Flow{
		{
			...
			generator: func(rep *report.Report) {
				...
				node3.AppendMetadata(1, 100, rep)
				node4.AppendMetadata(100, 2, rep)
				node5.AppendMetadata(1, 0, rep)
			},
			pktCount:  pktsPerFlow,
			prod:      prod,
			partition: kafka.TopicPartition{Topic: &topics[4], Partition: kafka.PartitionAny},
		},
		{
			key: &report.FlowKey{DstAs: 0xff00_0000_0005, SrcAs: 0xff00_0000_0002, FlowId: 0x02},
			generator: func(rep *report.Report) {
				node2.AppendMetadata(0, 1, rep)
				node3.AppendMetadata(1, 100, rep)
				node4.AppendMetadata(100, 2, rep)
				node6.AppendMetadata(1, 0, rep)
			},
			pktCount:  pktsPerFlow,
			prod:      prod,
			partition: kafka.TopicPartition{Topic: &topics[5], Partition: kafka.PartitionAny},
		},
	}
	...
	t0 := time.Now()
	for i := range flows {
		flows[i].start(&wg, time.Duration(1.0e6/reportRate)*time.Microsecond)
	}
	wg.Wait()
